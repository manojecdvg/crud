...
return split( term ).pop();
				...
				var oTable = $('#phonebook').dataTable( {
					...
					"sAjaxSource": '<?php echo base_url();?>index.php/phonebook/userAjax',
	...
	$( "#dialog-form-success-file1" ).dialog();								
		...
		$('td:eq(3)', nRow).html( '<a href="<?php base_url()?>index.php/phonebook/viewuc/'+aData[1]+'" class="contact">'+aData[3]+'</a>'  );
		$('td:eq(4)', nRow).html( '<a href="<?php base_url()?>index.php/phonebook/viewuc/'+aData[1]+'" class="contact">'+aData[4]+'</a>'  );	
...
$.ajax({
	...
	data:{'uid': uid},
	url: "<?php echo base_url()?>index.php/phonebook/getUser",
...
primary: "ui-icon-arrowreturn-1-w"
					...
					.click(function() {
						...
						$.ajax({
						        ...
						        url: "<?php echo base_url()?>index.php/phonebook/deleteUser",
						      success: function(data)
						        {  
               ...
               click: function(event) {
				   $.blockUI({ message: '<h2><img src="<?php echo base_url()?>/img/ajax-loader_2.gif" />please wait...</h2>' });  
							...
							$.ajax({
								...
								dataType:"json",
						        data:{'form': $("#frmadd").serialize()},
						        url: "<?php echo base_url()?>index.php/phonebook/addUser",
						        success: function(data)
						        {   
               ...
               click: function(event) {
				   $.blockUI({ message: '<h2><img src="<?php echo base_url()?>/img/ajax-loader_2.gif" />please wait...</h2>' });  
							...
							$.ajax({
						        ...
						        url: "<?php echo base_url()?>index.php/phonebook/editUser",
						        success: function(data)
						        {   
...
<p>
     ...
     <label for="first_name">User Name<font color="#FF0000"> *</font>
	   <select name="uid" id="uid" class="validate[required] select ui-widget-content ui-corner-all" data-prompt-position="topLeft" style="width:325px;">
            <option value="" >[Select One]</option>
            <?php
			$query = $this->doctrine->em->createQuery ( "SELECT d FROM Entity\\User d" );
		$users = $query->getResult ();
		
		foreach ( $users as $user ) {
			echo '<option value="' . $user->getUid () . '">' . $user->getFirstName ().' '.$user->getLastName () . '</option>';
		}
...
<button id="reject" class="cancelButton ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" >Delete</button>
	...
	<br>
	<br>
	<strong class="validateTips">click on Firstname/Last name to view user contacts </strong><br>
	<br>
