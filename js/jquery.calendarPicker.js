      ...
      for (var i = -options.days; i <= options.days; i++) {
        ...
        if (i == -options.days && options.showDayArrows) {
          ...
          span.addClass("next");
			...
			if(options.dayNames[d.getDay()]=='Sat' || options.dayNames[d.getDay()]=='Sun'  ||  options.holidays.indexOf(dt)>=0){
				
			span.html("<span class='dayNumber1' title='Non Working Day'>" + d.getDate() + "</span><br>" + options.dayNames[d.getDay()]).css("width",w);}
			else
